                                     ...
                                     const char* table_path, u32 unk)
{
    ...
    void* table_binary = reinterpret_cast<void*>(FileDeviceMgr::instance()->tryLoad(load_arg));

    load_arg.path = font_path;
    load_arg.alignment = 0x1000;
    load_arg.buffer_size_alignment = 0x1000;
    void* font_binary = reinterpret_cast<void*>(FileDeviceMgr::instance()->tryLoad(load_arg));
    u32 font_size = load_arg.read_size;

    load_arg.path = shader_path;
    void* shader_binary = reinterpret_cast<void*>(FileDeviceMgr::instance()->tryLoad(load_arg));
...
// missing initializeFromBinary and swapUniformBlockBuffer
...
void DebugFontMgrJis1Nvn::begin(DrawContext* ctx) const
{
    if (_530)
        return;
    pfnc_nvnCommandBufferBindProgram(ctx->getNvnCommandBuffer(), &nvnProgram, 31);
}
void DebugFontMgrJis1Nvn::end(DrawContext* ctx) const {}
...
void DebugFontMgrNvn::end(DrawContext* ctx) const {}

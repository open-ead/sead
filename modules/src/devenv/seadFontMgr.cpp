...
// SMO
// NON_MATCHING: Stack
void DebugFontMgrJis1Nvn::initializeFromBinary(Heap *heap, void *shader_binary, u64 shader_size, void *font_binary, u64 font_size, const void *table_binary, u32 unk3)
{
    ...
    nvnMemoryPoolBuilderSetFlags(&poolBuilder, NVNmemoryPoolFlags::NVN_MEMORY_POOL_FLAGS_CPU_UNCACHED | NVNmemoryPoolFlags::NVN_MEMORY_POOL_FLAGS_GPU_CACHED | NVNmemoryPoolFlags::NVN_MEMORY_POOL_FLAGS_SHADER_CODE);
    ...
    NVNshaderData data[2] {
        { bufferAddress + header->shaderDataOffset, reinterpret_cast<void*>(uintptr_t(shader_binary) + ptrdiff_t(header->offset1)) },
        { bufferAddress + header->offset4, reinterpret_cast<void*>(uintptr_t(shader_binary) + ptrdiff_t(header->offset2)) }
    };
    ...
    nvnMemoryPoolBuilderSetDefaults(&poolBuilder);
    nvnMemoryPoolBuilderSetDevice(&poolBuilder, device);
    nvnMemoryPoolBuilderSetFlags(&poolBuilder, NVNmemoryPoolFlags::NVN_MEMORY_POOL_FLAGS_CPU_UNCACHED | NVNmemoryPoolFlags::NVN_MEMORY_POOL_FLAGS_GPU_CACHED);
    ...
    nvnTextureBuilderSetTarget(&texBuilder, NVNtextureTarget::NVN_TEXTURE_TARGET_2D);
    nvnTextureBuilderSetFormat(&texBuilder, NVNformat::NVN_FORMAT_RGTC1_UNORM);
    nvnTextureBuilderSetSize2D(&texBuilder, 1280, 1298);
    nvnTextureBuilderSetPackagedTextureData(&texBuilder, reinterpret_cast<void*>(uintptr_t(font_binary) + 0x200));

    nvnMemoryPoolBuilderSetDefaults(&poolBuilder);
    nvnMemoryPoolBuilderSetDevice(&poolBuilder, device);
    nvnMemoryPoolBuilderSetFlags(&poolBuilder, NVN_MEMORY_POOL_FLAGS_CPU_NO_ACCESS | NVN_MEMORY_POOL_FLAGS_GPU_CACHED);
    ...
    CriticalSection* criticalSection2 = static_cast<GraphicsNvn*>(sead::Graphics::instance())->getCriticalSection2();
    criticalSection2->lock();
    int newTextureId = static_cast<GraphicsNvn*>(sead::Graphics::instance())->getNewTextureId();
    nvnTexturePoolRegisterTexture(static_cast<GraphicsNvn*>(sead::Graphics::instance())->getTexturePool(), newTextureId, &mNvnTexture, nullptr);
    criticalSection2->unlock();
    mNvnTextureHandle = nvnDeviceGetTextureHandle(device, newTextureId, static_cast<GraphicsNvn*>(sead::Graphics::instance())->getTextureSamplerID());
}

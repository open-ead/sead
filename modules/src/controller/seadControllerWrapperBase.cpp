...
ControllerWrapperBase::ControllerWrapperBase()
    : ControllerBase(Controller::cPadIdx_Max, -1, -1, Controller::cPadIdx_Touch), IDisposer(),
      mController(NULL), mIsEnable(true), mListNode()
{
}
...
void ControllerWrapperBase::setEnableOtherWrappers(bool enable) const
{
    ...
    for (OffsetList<ControllerWrapperBase>::iterator it = wrappers.begin(); it != wrappers.end();
         ++it)
    {
